function plotstat(fname)
  ...
  mmean = c{4};
  mmax = c{5};
  % get average server ping and jitter:
  pinglat = nan+zeros(1,numel(csLabels));
  jitter = nan+zeros(1,numel(csLabels));
  lat = nan+zeros(1,numel(csLabels));
  for k=1:numel(pinglat)
    idx = find(isfinite(mmean).*(mcaller==(k-1)));
    pinglat(k) = mean(mmean(idx));
    jitter(k) = mean(mmax(idx)-mmean(idx));
    lat(k) = mean(mmean(idx)+jitterbuf+hwdelay)*0.5;
  end
  %
